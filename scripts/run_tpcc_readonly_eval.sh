...
# Pure read-only TPC-C evaluation script
# Runs 100% OrderStatus transactions (read-only scans)
# Compares fast-path vs baseline performance on pure read workload

NSHARDS=${NSHARDS:-1}
SHARD_INDEX=${SHARD_INDEX:-0}
THREADS=${THREADS:-6}
RUNTIME=${RUNTIME:-60}
RESULT_DIR=${RESULT_DIR:-results/tpcc_readonly_eval}
WAIT_FOR_METRIC_SECS=${WAIT_FOR_METRIC_SECS:-30}
FOLLOWER_READY_TIMEOUT=${FOLLOWER_READY_TIMEOUT:-120}

# 100% OrderStatus (read-only scans): NewOrder, Payment, Delivery, OrderStatus, StockLevel
# 0,0,0,100,0 = 0% NewOrder, 0% Payment, 0% Delivery, 100% OrderStatus, 0% StockLevel
export MAKO_TPCC_WORKLOAD_MIX="${MAKO_TPCC_WORKLOAD_MIX:-0,0,0,100,0}"
...
log "Starting TPCC read-only evaluation"
...
echo "EVAL_RESULT workload=tpcc_readonly nshards=$NSHARDS shard=${SHARD_INDEX} threads=$THREADS runtime_s=$RUNTIME workload_mix=${MAKO_TPCC_WORKLOAD_MIX} fast_ro_mode=${MAKO_FAST_RO_MODE:-baseline} agg_persist_throughput=${throughput}"
